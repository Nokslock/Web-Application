...
export default function ProfileForm({ user }: ProfileFormProps) {
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setLoading(true);
    // toast.loading returns an ID we can use to dismiss it later
  ...
  return (
    <div className="w-full border border-neutral-200 rounded-xl p-8 relative">
      {/* --- MAIN CONTENT --- */}
            ...
            <div className="flex justify-between items-center">
              ...
              {isEditingName ? (
                 <div className="flex gap-2">
                    <button onClick={() => setIsEditingName(false)} className="text-xs text-red-500 font-medium hover:underline">Cancel</button>
                    <button onClick={handleUpdateName} disabled={loading} className="text-xs text-blue-600 font-bold hover:underline">Save</button>
                 </div>
              ) : (
                <button onClick={() => setIsEditingName(true)} className="text-xs text-blue-600 font-medium flex items-center gap-1 hover:underline">
...
// --- SUB-COMPONENTS (With Toast Notifications) ---
        ...
        {step === 1 ? (
          ...
          <div className="space-y-4">
            ...
            <input 
              ...
              placeholder="000000"
              maxLength={6}
              className="w-full p-3 border rounded-lg text-center tracking-widest text-xl font-bold"
              value={otp}
