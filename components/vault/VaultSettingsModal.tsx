...
import { updateVaultSettings } from "@/lib/vault-actions";
...
export default function VaultSettingsModal({ vault, onClose, onUpdate }: VaultSettingsModalProps) {
  ...
  const [lockCode, setLockCode] = useState(""); // Always blank — set-new-only, never pre-populated
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) return;
    if (isLocked && !vault.is_locked && lockCode.length < 4) {
      // Only require code length when ENABLING lock for the first time or changing it
      toast.error("Lock code must be at least 4 characters.");
      return;
    }
    if (isLocked && lockCode && lockCode.length < 4) {
      toast.error("Lock code must be at least 4 characters.");
      return;
    }
    ...
    try {
      const result = await updateVaultSettings({
        ...
        lockCode,
        shareWithNok,
        previousShareWithNok: vault.share_with_nok,
      });

      if (!result.success) throw new Error(result.error);

      if (result.nokRevokeFailed) {
        toast.warning("Vault updated, but some items could not be unshared.");
      } else if (vault.share_with_nok && !shareWithNok) {
        toast.success("Revoked NOK access from all vault items.");
      }
          ...
          {showDeleteConfirm ? (
            ...
            <form onSubmit={handleUpdate} className="space-y-5">
                  ...
                  {isLocked && (
                    ...
                    >
                      <input
                        ...
                        placeholder={vault.is_locked ? "Enter new lock code…" : "Enter lock code…"}
              ...
              <div
                ...
                className={`cursor-pointer flex items-center gap-3 p-4 rounded-xl border transition-all duration-200 ${shareWithNok
                  ? "bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800"
                  : "bg-gray-50 dark:bg-gray-800 border-gray-100 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700"
                  }`}
              >
                <div className={`shrink-0 w-10 h-10 rounded-lg flex items-center justify-center transition-colors ${shareWithNok ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-400 dark:bg-gray-700"
                  }`}>
                  <FaUserShield />
                </div>
                <div className="flex-1">
